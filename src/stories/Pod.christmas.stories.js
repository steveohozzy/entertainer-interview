...
import { Podchristmas } from './Podchristmas';
import { useArgs } from 'storybook/preview-api';

// More on how to set up stories at: https://storybook.js.org/docs/writing-stories#default-export
export default {
  title: 'Home/Pods/Pod Chrismas',
  component: Podchristmas,
  argTypes: {
    position: {
...
export const PodItemChristmas = {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const docRef = doc(db, args.user, "podchristmas");
  ...
  useEffect(() => {
    ...
    const send = async () => {
      try {
        const docRef = doc(db, selectedUser, "podchristmas");
