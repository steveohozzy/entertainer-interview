...
import { ChristmasPods3x2 } from './Christmaspods3x2';
import { useArgs } from 'storybook/preview-api';

// More on how to set up stories at: https://storybook.js.org/docs/writing-stories#default-export
export default {
  title: 'Christmas/Pods',
  component: ChristmasPods3x2,
  parameters: {
...
export const ChristmasPods3x2Section = {
  args: {
    user: 'stories',
    pod1image: '',
    pod1alt: '',
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const docRef = doc(db, args.user, "christmaspods3x2");
  ...
  useEffect(() => {
    ...
    const send = async () => {
      try {
        const docRef = doc(db, selectedUser, "christmaspods3x2");
