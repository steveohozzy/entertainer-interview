...
import { ShopByAgeChristmas } from './Shopbyagechristmas';
import { useArgs } from 'storybook/preview-api';

export default {
title: 'Christmas/Shop By Age',
component: ShopByAgeChristmas,
parameters: {
layout: 'fullscreen',
},
...
useEffect(() => {
  ...
  const load = async () => {
    ...
    try {
      const ref = doc(db, 'ShopByAgeChristmas', currentArgs.selectedModule);
...
useEffect(() => {
  ...
  const save = async () => {
    try {
      ...
      await setDoc(
        doc(db, 'ShopByAgeChristmas', moduleName),
        fields,
        {
          merge: false,
        }
  ...
  const loadModules = async () => {
    try {
      const snap = await getDocs(
        collection(db, 'ShopByAgeChristmas')
      );
...
return (
  <>
    ...
    <ShopByAgeChristmas {...componentArgs} />
  </>
);
