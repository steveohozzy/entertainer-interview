...
const MiniCart = () => {
  ...
  return (
    <>
      {isCartOpen && (
            <div className="fixed h-screen inset-0 bg-black bg-opacity-50 z-40 top-[44px] md:top-[72px] left-[-20px]" onClick={() => toggleIsCartOpen(false)} />
        )}
      <div className={`absolute max-w-[90vw] overflow-hidden w-[320px] transition-all right-0  ${isCartOpen ? 'max-h-[calc(100vh-100px)]' : 'max-h-0'} top-full md:top-[calc(100%+10px)] z-[9999] bg-white rounded-xl text-black`}>
        <div className="max-h-[calc(100vh-100px)] overflow-y-auto w-full px-3">
          ...
          <div>
            {cartItems.length ? (
                cartItems.map((item) => <CartProductTile key={item.id} product={item} isMiniCart={true} />)
            ) : (
              <div className="py-3 text-brandBlue font-bold text-lg">Your cart is empty</div>
            )}
          </div>
          ...
          <div className='w-full text-center mt-4'>
            <HeadingRibbon smallerText={true}>
                <button className="flex items-center" onClick={() => setRecsDetailsOpen(!recsDetailsOpen)}>You may also like... <span
                ...
                <div className='grid grid-cols-3 gap-3 max-w-[800px] mx-auto'>
                  {recs.map((product) => (
                    <div className="flex flex-col h-full" key={product.id}>
                      ...
                      />
                      <div className={`grid overflow-hidden grow transition-all duration-300 ease-in-out text-slate-600 text-sm ${recsDetailsOpen
          ...
          <div className='bg-white w-full sticky bottom-0 py-2'>
            <Button 
